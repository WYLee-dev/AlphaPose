# pytorch-AlphaPose 변경 사항
 ...
 - SPPE/src/models/layers/SE_module.py 파일의 코드 변경(기존 코드로 ONNX 변환은 가능하지만, TensorRT에서 불러오지 못함)
 - SPPE/src/main_fast_inference.py 파일의 코드 변경(pretrained model의 경로를 직접 설정하도록 변경)
